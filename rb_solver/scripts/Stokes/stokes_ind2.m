function [ E, A, B, C, v1xx, v2xx, pxx, nf ] = stokes_ind2(m, q, nx, ny)
% *** Taken from https://www.mpi-magdeburg.mpg.de/projects/mess ***
...
v1xx = [v1x1,v1x2];
v2xx = [v2x1;v2x2]';
pxx  = [repmat(x1,[1,ny]); repelem(x2,nx)]';

E    = sparse(nv1+nv2+np-1,nv1+nv2+np-1);
B    = sparse(nv1+nv2+np-1,m);
C    = sparse(q,nv1+nv2+np-1);
viskos = 1;
A = [viskos*L1 sparse(nv1,nv2) -G1(:,1:np-1);
     sparse(nv2,nv1) viskos*L2 -G2(:,1:np-1);
     D1(1:np-1,:) D2(1:np-1,:) sparse(np-1,np-1)];
pxx(end,:) = [];

% E    = sparse(nv1+nv2+np,nv1+nv2+np);
% B    = sparse(nv1+nv2+np,m);
% C    = sparse(q,nv1+nv2+np);
% A = [L1 sparse(nv1,nv2) -G1(:,1:np);
%      sparse(nv2,nv1) L2 -G2(:,1:np);
%      D1(1:np,:) D2(1:np,:) sparse(np,np)];
